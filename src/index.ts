...
class Yiff {
  ...
  constructor(config?: config) {
    ...
    this.useragent = this.config?.useragent
      ? `${defaults.useragent} ${this.config.useragent}`
      : defaults.useragent;
    console.log(config);
    this.killswitch = {
  ...
  async e621(tags: string | string[], limit?: number, page?: number) {
    try {
      if (this.config?.apikey?.e621) {
        ...
        throw new Error("e621 API Key is required in config.apikey.e621");
      }
    } catch (error) {
      console.error(error);
    }
